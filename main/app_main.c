...
int publish_device_shadow(bytebeam_client_handle_t client, bytebeam_client* bb_obj)
{
	...
	cJSON_AddItemToObject(demo_data_json, "sequence", seq_id_json);
	char temp_buff[200];
	const char* led_status_to_send[1]={"dummy"};
	sprintf(temp_buff, "LED is %s!", s_led_state == true ? "ON" : "OFF");
	led_status_to_send[0]=temp_buff;
	// device_status_json = cJSON_CreateStringArray(led_status_to_send,1);
	...
	cJSON_AddItemToObject(demo_data_json,"Status", device_status_json);
