...
void do_something()
{
    ...
    // log to serial 
    ESP_LOGI(TAG, "crash_counter : %d", crash_counter);

    // if the counter reaches the threshold, break the code
    if(crash_counter == threshold) {
        ESP_ERROR_CHECK(-1);
    }
}

void coredump_test_task_1(void *pvParameters) 
{
    while(1) {
        xSemaphoreTake( xSemaphore, portMAX_DELAY );
        ESP_LOGI(TAG, "Task 1 Calling");
        do_something();
...
void coredump_test_task_2(void *pvParameters) 
{
    while(1) {
        xSemaphoreTake( xSemaphore, portMAX_DELAY );
        ESP_LOGI(TAG, "Task 2 Calling");
        do_something();
        xSemaphoreGive( xSemaphore );
        vTaskDelay(1000 / portTICK_PERIOD_MS);
    }
...
void app_main(void)
{
    ...
    if (xSemaphore == NULL) {
        ESP_LOGE(TAG, "Failed to create semaphore handle");
        return;
    }

    xSemaphoreGive( xSemaphore );

    ESP_LOGI(TAG, "threshold : %d", threshold);

    xTaskCreate(&coredump_test_task_1, "coredump_test_task_1", 8192, NULL, 5, NULL);
    xTaskCreate(&coredump_test_task_2, "coredump_test_task_2", 8192, NULL, 5, NULL);
